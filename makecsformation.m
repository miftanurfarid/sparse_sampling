...
% location of the transmitter or receiver in the 2D plane ...(x+iy) in
% meters.
...
% measurement. Must be nmax by nmax. (randn(600,600) used ...throughout
% research)
% dataparams.keepsamples ? a 1xG cell array where each entry is a row
% vector of the same length (<nmax) listing the integer valued ...entries of
% each effective measurement to keep. (for research, these were
% pre?determined randomly as 150 samples out of the 600 for each
% geometry) ? All cell elements must be vectors of the same ...length!!
%
% imageparameters.L ? length in meters of the unknown scene. ...Assumed to
% be square (LxL meters)
% imageparameters.N ? number of pixels along one edge of the unknown
% scene. Final modeled image is NxN pixels square.
%
%%Output Parameters
% A ? MxN matrix (M = (number of geometries)(length of effective
% measurement) and N = (imageparameters.N)ˆ2
...
%make cell vector for just the used data (maps from ftx,rxg to fgg)
for k = 1:dataparams.G
Adatafkg = adata.datafdataparams.g(k,1),dataparams.g(k,2)g;
if (length(Adatafkg)<dataparams.nmax) Adatafkg = [Adatafkg ...
zeros(1,length(dataparams.nmax-Adatafkg))]; end
Ydatafkg = ydata.datafdataparams.g(k,1),dataparams.g(k,2)g;
if (length(Ydatafkg)<dataparams.nmax) Ydatafkg = [Ydatafkg ...
zeros(1,length(dataparams.nmax-Ydatafkg))]; end
end
...
for g = 1:G
FreqDatafgg = fft(Adatafgg(1:nmax),nmax);
end
...
for pixel=1:N^2
if (mod(pixel,fix(N^2/100))==0) fprintf('%d ',pixel); end
for g = 1:G
tau = round(delayfgg(pixel))/fs;
e = exp(j*2*pi.*f*-tau);
%apply shift in time in freq domain
FreqD = e.*FreqDatafgg;
%sample based on input parameters from user
datapoints = T*real(ifft(FreqD))';
datapoints = datapoints(round(keepsamplesfgg));
...
datapoints = (dataparams.TM*Ydatafgg(1:dataparams.nmax)');
datapoints = datapoints(round(dataparams.keepsamplesfgg));
datapoints = datapoints./max(datapoints);
